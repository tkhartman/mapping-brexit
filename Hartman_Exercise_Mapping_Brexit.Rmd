...
To make a map, we first need to draw the boundaries so that we can overlay the points or shading from our spatial data analysis. R has libraries of these maps in avarious; however, not all maps are available. Sometimes it's necessary to load what's call a *shapefile*, which contains the longitude and latitude for all of the borders of the map we want to draw. This is what we'll need to use to draw the EU Referendum constituencies (excluding Gibraltar).

The shapefile (.shp) can be downloaded from Github (it's originally from Ordinance Survey). Make sure the file is downloaded to your working directory or R will be unable to import it. We're going to be using the `read_sf()` function from the `sf` package. The code below will automate this process (download the file, unzip it, and import the .shp into R), but you can do this manually if you encounter any errors.

```{r, echo=TRUE}
## Download location and file name
url.shp <- "https://github.com/tkhartman/mapping-brexit/raw/master/esri.zip"
...
## Unzip to a folder named 'shp'
file.shp <- unzip(file.zip, exdir = "shp")
...
Now we're ready to create the basic map using `ggplot`. It's a powerful package for data visualization written by Hadley Wickham. The plot takes a minute or two to complete, so be patient! It should be saved as a .pdf file in your working directory (if it appears blank, try opening it with Adobe Reader).

```{r, echo=TRUE}
## Mapping Brexit vote share using ggplot
brexit.map <- ggplot(uk.df) + 
    geom_sf(aes(fill = Pct_Leave)) 
